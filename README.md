...
## 进阶用法：把 nudge 写成工具触发指令

因为 `[nudge]` 本质上是一条用户消息，所以它不只能是“提醒一下我”。用户可以把它预先写成更具体的指令，让模型主动去调用已有工具或拉取数据，例如：

```txt
已经有一段时间没说话了。请先查看 heartbeat 判断我在哪里、手机在做什么；再看最近待办或日程；最后根据上下文自然地发一条消息，不要提到这是自动触发。
```

或者：

```txt
夜班检查：请查看最近日记、ledger 待办、服务器状态和市场价格，有重要异常再提醒我；没有异常就写一条简短的陪伴消息。
```

这样 proactive worker 不需要理解 heartbeat、vault、todo、ledger、calendar 这些业务数据，也不需要额外把数据塞进 prompt。它只负责到点把用户自定义指令送进聊天入口；具体要查什么、怎么调用工具、怎么组织回复，都交给原本的模型和工具系统。

这个设计的好处是：业务能力都留在 chat gateway / tools 里，proactive 只是一个很薄的触发层。

## 取舍：直接注入数据 vs 让模型自己查

如果目标只是固定地附带一小段数据，比如当前状态、最近待办、天气、位置，直接在 worker 里把数据查好并塞进 `[nudge]` 或 system context 里，通常会更省：少一次模型工具调用，延迟也更低。

但这种做法会让 proactive worker 越来越懂业务。每加一个数据源，都要改 worker 的代码或配置；数据格式变了，也要跟着改。

让 `[nudge]` 保持为用户自定义消息，再让模型自己调用工具，通常会多一次 API 往返，token 和延迟不一定最省。但它更灵活：用户可以随时把注入内容改成“先查 heartbeat 再说话”“先看待办再提醒”“夜班时巡服务器”等，不需要改 worker。worker 只负责触发，业务逻辑留在 gateway 和 tools 里。

一个实用判断：

- 固定、轻量、稳定的数据：可以直接注入，更省。
- 数据源经常变化、逻辑复杂、用户想自己改触发词：让模型按自定义 nudge 去查，更灵活。
